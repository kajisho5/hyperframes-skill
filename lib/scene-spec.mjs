...
export const COLOR_RE = /^(#[0-9a-fA-F]{3,4}|#[0-9a-fA-F]{6}|#[0-9a-fA-F]{8}|transparent|rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*(,\s*(0|1|0?\.\d+)\s*)?\))$/;
const FONT_RE = /^[A-Za-z0-9 ,'_-]{1,200}$/;
const IMAGE_EXT = new Set([".png", ".jpg", ".jpeg", ".webp", ".gif", ".svg"]);
const VIDEO_EXT = new Set([".mp4", ".webm", ".mov", ".m4v"]);
const FITS = ["contain", "cover", "fill"];

const TOP_KEYS = new Set(["scene_version", "id", "width", "height", "duration", "fps", "background", "lang", "fonts", "layers"]);
// Fonts (0.5.0): font files shipped inside the scene and declared with @font-face, so text renders
// with that font on any machine instead of whatever the machine's fontconfig picks.
...
const LAYER_KEYS = {
  ...
  video: new Set(["type", "id", "start", "duration", "box", "src", "fit", "media_start", "volume", "transition_in", "transition_out"]),
  audio: new Set(["type", "id", "start", "duration", "src", "media_start", "volume", "fade_in", "fade_out"]),
};
// Audio (0.7.0). volume is linear gain as HyperFrames applies it: 1 = unchanged (0 dB), 0.5 =
// -6 dB, up to 3.98 (+12 dB, HyperFrames' ceiling). A video layer without `volume` stays muted.
const AUDIO_EXT = new Set([".wav", ".mp3", ".m4a", ".aac", ".ogg", ".opus", ".flac"]);
export const MAX_VOLUME = 3.98;
...
export function validateRequest(req, baseDir) {
  ...
  const fonts = [];
  if (req.fonts !== undefined && !Array.isArray(req.fonts)) err("fonts must be an array");
  (Array.isArray(req.fonts) ? req.fonts : []).forEach((F, i) => {
    const at = `fonts[${i}]`;
    if (typeof F !== "object" || F === null) return err(`${at} must be an object`);
    for (const k of Object.keys(F)) if (!FONT_KEYS.has(k)) err(`${at}: unknown key "${k}"`);
    if (typeof F.family !== "string" || !FONT_FAMILY_RE.test(F.family)) err(`${at}.family must match ${FONT_FAMILY_RE} (the name layers use in style.font_family)`);
    const weight = F.weight ?? null;
    if (weight !== null && (!isInt(weight) || weight < 100 || weight > 900 || weight % 100)) err(`${at}.weight must be 100, 200, ..., 900 (omit it to use this file for every weight)`);
    const style = F.style ?? "normal";
    if (!["normal", "italic"].includes(style)) err(`${at}.style must be normal or italic`);
    let srcAbs = null;
    if (typeof F.src !== "string" || F.src === "") err(`${at}.src must be a path to a local font file`);
    else if (/^[a-z][a-z0-9+.-]*:/i.test(F.src) && !/^[A-Za-z]:[\\/]/.test(F.src)) err(`${at}.src must be a local file path, not a URL (this skill never fetches)`);
    else {
      srcAbs = isAbsolute(F.src) ? F.src : resolve(baseDir, F.src);
      const ext = extname(srcAbs).toLowerCase();
      if (!FONT_EXT.has(ext)) err(`${at}.src: font extension must be one of ${[...FONT_EXT].join(" ")}`);
      let isFile = false;
      try {
        isFile = statSync(srcAbs).isFile();
      } catch {
        /* reported below */
      }
      if (!isFile) err(`${at}.src: no such file: ${srcAbs}`);
      fonts.push({ family: F.family, srcAbs, assetName: `fonts/font-${i + 1}${ext}`, weight, style });
    }
  });
  ...
  (Array.isArray(req.layers) ? req.layers : []).forEach((L, i) => {
    ...
    if (!allowed) return err(`${at}.type must be one of text, image, video, audio`);
    ...
    const box = L.type === "audio" ? null : L.box ?? { x: 0, y: 0, width: req.width, height: req.height };
    if (L.type === "audio") {
      /* not visual: no box */
    } else if (typeof box !== "object" || box === null) err(`${at}.box must be an object`);
    else {
    ...
    if (L.type === "text") {
      ...
      if (typeof L.src !== "string" || L.src.length === 0) err(`${at}.src must be a path to a local file`);
        ...
        const okExt = L.type === "image" ? IMAGE_EXT : L.type === "audio" ? AUDIO_EXT : VIDEO_EXT;
      ...
      if (L.type === "video" || L.type === "audio") {
        ...
        if (L.volume !== undefined || L.type === "audio") {
          const volume = L.volume ?? 1;
          if (!isNum(volume) || volume < 0 || volume > MAX_VOLUME) err(`${at}.volume must be a number 0..${MAX_VOLUME} (linear gain: 1 = unchanged, 0.5 = -6 dB)`);
          out.volume = volume;
        }
      }
      if (L.type === "audio") {
        for (const k of ["fade_in", "fade_out"]) {
          if (L[k] !== undefined && (!isNum(L[k]) || L[k] < 0)) err(`${at}.${k} must be a number of seconds >= 0`);
          out[k] = L[k] ?? 0;
        }
        if (isNum(out.fade_in) && isNum(out.fade_out) && isNum(L.duration) && ms(out.fade_in) + ms(out.fade_out) > ms(L.duration)) err(`${at}: fade_in + fade_out is longer than the layer`);
      }
    }
  ...
  return { id, width: req.width, height: req.height, duration: req.duration, fps, background, lang, fonts, layers };
}
...
export function renderHtml(scene) {
  const { id, width: W, height: H, duration, fps, background, lang, fonts = [], layers } = scene;
  const css = [
  ...
  for (const L of layers) {
    const sel = `#${L.id}`;
    const b = L.box;
    let rule = b ? `left:${b.x}px;top:${b.y}px;width:${b.width}px;height:${b.height}px;z-index:${L.z}` : "";
    ...
    if (L.type === "text") {
      ...
      body.push(
        L.volume === undefined
          ...
          : `      <video id="${L.id}" class="clip" src="assets/${esc(L.assetName)}" playsinline preload="auto" ${timing} data-media-start="${sec(L.media_start)}" data-volume="${sec(L.volume)}" data-has-audio="true"></video>`,
      );
    } else {
      // audio: not painted, so no CSS rule
      const fades = `${L.fade_in ? ` data-fade-in="${sec(L.fade_in)}"` : ""}${L.fade_out ? ` data-fade-out="${sec(L.fade_out)}"` : ""}`;
      body.push(`      <audio id="${L.id}" class="clip" src="assets/${esc(L.assetName)}" preload="auto" ${timing} data-media-start="${sec(L.media_start)}" data-volume="${sec(L.volume)}"${fades}></audio>`);
      continue;
    }
  ...
  return [
    ...
    "    <style>",
    // weight omitted: one file for every weight (100 900), so the browser neither synthesizes
    // bold nor falls back to another face for a weight the request did not declare
    ...fonts.map((F) => `      @font-face{font-family:"${F.family}";src:url("assets/${F.assetName}");font-weight:${F.weight ?? "100 900"};font-style:${F.style}}`),
    ...css.map((l) => "      " + l),
    "    </style>",
    "  </head>",
    "  <body>",
