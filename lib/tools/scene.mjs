...
export async function run(ctx, [requestPath], opts) {
  ...
  const assets = media.map((L) => ({ layer: L.id, source: L.srcAbs, path: join(dir, "assets", L.assetName) }));
  ...
  for (const a of assets) copyFileSync(a.source, a.path);
  if (assets.length === 0) rmSync(join(dir, "assets"), { recursive: true });
