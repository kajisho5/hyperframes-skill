...
export function planBatch({ rowsFile, templateName, outDir, nameField, encoding, codec, overwrite }) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      request = fillTemplate(template, values, omitted);
    } catch (e) {
...
export async function run(ctx, [rowsFile], opts) {
  ...
  const plan = planBatch({ rowsFile, templateName: opts.template, outDir, nameField: opts.nameField, encoding: opts.encoding, codec: opts.codec, overwrite: !!opts.overwrite });
  ...
  for (const p of plan.planned) {
    ...
    try {
      ...
      writeSceneDir(p.sceneDir, renderHtml(scene), []);
