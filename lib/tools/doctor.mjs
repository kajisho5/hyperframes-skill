...
import { MIN_FFMPEG_MAJOR, MIN_NODE_MAJOR, parseFfVersion, PKG, REQUIREMENTS, resolveChromium, resolveFfTool, resolveHyperframes, which } from "../deps.mjs";
...
export const meta = {
  ...
  output_schema: "{status, ok, exit_code, capabilities: {<name>: {status: available|missing|unknown, detail, ...}}, tools: {<tool>: {usable: yes|no|unknown, missing?, unknown?}}, fonts: {scripts: {ja|zh-cn|ko: {status, family, file, fonts_covering, detail}}, advice} (informational, never affects ok), requirements, commands}",
};
...
// CJK coverage from fontconfig, which is what headless Chrome on Linux asks for fallback fonts.
// Informational: it never changes `ok` or a tool's `usable`; a scene that ships its font
// (request `fonts`, or --font on template / batch) does not depend on it at all.
const FONT_LANGS = { ja: "Japanese", "zh-cn": "Chinese (Simplified)", ko: "Korean" };

function fontInfo(ctx) {
  const fcList = which("fc-list");
  const fcMatch = which("fc-match");
  const scripts = {};
  if (!fcList || !fcMatch) {
    for (const [lang, label] of Object.entries(FONT_LANGS)) scripts[lang] = { status: "unknown", detail: `${label}: fontconfig (fc-list / fc-match) not on PATH; coverage could not be checked` };
    return { scripts, advice: "ship the font with the scene (request `fonts`, or --font on template / batch) to not depend on the machine's fonts" };
  }
  for (const [lang, label] of Object.entries(FONT_LANGS)) {
    const list = probeRun(ctx, [fcList, `:lang=${lang}`, "file"], { timeoutMs: 30000 });
    const match = probeRun(ctx, [fcMatch, "-f", "%{family[0]}|%{file}", `:lang=${lang}`], { timeoutMs: 30000 });
    if (!list.ok || !match.ok) {
      scripts[lang] = { status: "unknown", detail: `${label}: fontconfig failed (${(list.stderr || match.stderr || "").trim().slice(0, 200)})` };
      continue;
    }
    const covering = list.stdout.split("\n").filter(Boolean).length;
    const [family, file] = match.stdout.split("|");
    scripts[lang] = covering
      ? { status: "available", family, file, fonts_covering: covering, detail: `${label}: ${covering} font file(s); fontconfig picks ${family}` }
      : { status: "missing", family: null, file: null, fonts_covering: 0, detail: `${label}: no installed font covers it; text would render as boxes unless the scene ships a font` };
  }
  let advice = null;
  const ja = scripts.ja;
  const zh = scripts["zh-cn"];
  if (ja.status === "available" && zh.status === "available" && ja.file === zh.file) {
    advice = `fontconfig picks the same font (${ja.family}) for Japanese and Chinese, so Japanese text drawn with system fonts may use Chinese glyph forms whatever \`lang\` says; ship a Japanese font with the scene (request \`fonts\`, or --font on template / batch)`;
  } else if (ja.status !== "available") {
    advice = "no Japanese font is known to be installed; ship one with the scene (request `fonts`, or --font on template / batch)";
  }
  return { scripts, advice };
}
...
export async function run(ctx, _args, opts) {
  ...
  if (ctx.json) printJson(doc);
    ...
    for (const [k, v] of Object.entries(fonts.scripts)) process.stdout.write(`${v.status.padEnd(9)} ${("font:" + k).padEnd(20)} ${v.detail} (informational)\n`);
    if (fonts.advice) process.stdout.write(`note: ${fonts.advice}\n`);
